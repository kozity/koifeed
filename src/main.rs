...
impl Config {
    ...
    // TODO
    // Arguments are error-checked here; unwrap() should be used within these constituent functions.
...
fn main() -> () {
    match Config::init() {
        ...
        Err(Error::KeyNoMatch) => eprintln!("error: given feed_key did not match any feeds"),
        Err(Error::Xml(e)) => eprintln!("error (xml-rs): {:?}", e),
        _ => eprintln!("error: an error was encountered that the developer did not anticipate, but should've"),
    };
    process::exit(1);
}
